...
% K : stiffness matrix
% Overloaded for multiple C_bending values, provide C_shear in a cell
...
% stiffness matrix bending
    if iscell(C_bending)
        K(elementDof,elementDof)=K(elementDof,elementDof)+ ...
                B_b'*C_bending{1,e}*B_b*gaussWeights(q)*det(Jacob)*an(e);
            
    else
        K(elementDof,elementDof)=K(elementDof,elementDof)+ ...
                B_b'*C_bending*B_b*gaussWeights(q)*det(Jacob)*an(e);
...
if iscell(C_shear)
    K(elementDof,elementDof)=K(elementDof,elementDof)+...
        B_s'*C_shear{1,e}  *B_s*gaussWeights(q)*det(Jacob);  
else
    K(elementDof,elementDof)=K(elementDof,elementDof)+...
