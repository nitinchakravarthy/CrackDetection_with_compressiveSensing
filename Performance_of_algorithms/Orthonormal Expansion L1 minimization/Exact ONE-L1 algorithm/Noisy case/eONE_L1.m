function [x_rec, iter_tot iter] = eONE_L1(A_dict,Omega,b,r,Tol,maxIter,verbose)
...
bfull(Omega,:) =  b;
fb = (A_dict);
Atb = (conj(dftmtx(100))/100)*(pinv(fb)*b)*length(bfull);
%%%%% mtimes_end %%%
...
while ~converged && k < maxiter
    ...
    while ~convergedInner && j < maxiterInner
        ...
        %bfull(Omega,:) =  y_temp;
        fb = dftmtx(30)*(A_dict);
        A_y_temp =  (conj(dftmtx(100))/100)*(pinv(fb)*y_temp)*length(bfull);
        %%%%% mtimes_end %%%
        temp_x = x + A_y_temp;
